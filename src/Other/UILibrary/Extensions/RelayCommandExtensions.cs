...
public static class RelayCommandExtensions
{
    ...
    /// </summary>
    private static readonly ConditionalWeakTable<DependencyObject, PropertyChangeNotifier> notifiers = new ConditionalWeakTable<DependencyObject, PropertyChangeNotifier>();

    /// <summary>
    /// A mapping of registered buttons to their subscribed handlers.
    /// </summary>
    private static readonly ConditionalWeakTable<DependencyObject, SubscriptionData> handlers = new ConditionalWeakTable<DependencyObject, SubscriptionData>();
    ...
    {
        if (!(d is DependencyObject obj)) return;
        if (!(obj is ICommandSource)) return;
    ...
    private static void Command_CanExecuteChanged(object dummy)
    {
        ...
        if (Monitor.IsEntered(dummy)) return;
